...
export class ServicePackageComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.servicePackageService
      ...
      .subscribe(
        (res: HttpResponse<IServicePackage[]>) => {
  ...
  trackId(index: number, item: IServicePackage): number {
    return item.id!;
  }

  delete(servicePackage: IServicePackage): void {
    const modalRef = this.modalService.open(ServicePackageDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IServicePackage[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/service-package'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
